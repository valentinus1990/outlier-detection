...
public class CoolingLoadDBDaoImpl {
	...
	private static CoolingLoadDBDaoImpl instance;

	
	private CoolingLoadDBDaoImpl(){
		factory = Persistence.createEntityManagerFactory(PERSISTENCE_UNIT_NAME);
		em = factory.createEntityManager();
	}
	
	public static CoolingLoadDBDaoImpl getInstance(){
		if(instance == null){
			instance = new CoolingLoadDBDaoImpl();
		}
		return instance;
	}
	...
	public List<Map<Timestamp, Float>> getHistMissData() throws ParseException{
		...
		List<Timestamp> missDate = ft.collectHistMissDate();
...
//			System.out.println(element);
			...
			if(tempT.size() == 0){
				...
				//Basically only one element in tempT
				for(PermanentHistoryData data_T : tempT){
					if(data_T.getHistValue() < 200)
						T_t.put(Utility.converToDateHH(data_T.getId().getHistDateTime()), data_T.getHistValue());
					else{
						T_t.put(Utility.converToDateHH(data_T.getId().getHistDateTime()), (float) 0);
					}
				}
			}
			...
			if(tempRH.size() == 0){
				...
				for(PermanentHistoryData data_RH : tempRH){
					if(data_RH.getHistValue() < 200)
						RH_t.put(Utility.converToDateHH(data_RH.getId().getHistDateTime()), data_RH.getHistValue());
					else{
						RH_t.put(Utility.converToDateHH(data_RH.getId().getHistDateTime()), (float) 0);
					}
				}
			}
		}
	 ...
	 * This method is just used for setting up forecast data structure
	 * 
	 * @return
	 * @throws ParseException
	 */
	public List<Map<Timestamp, Float>> getLastWeekMissData() throws ParseException{
		FaultTolerant ft =  new FaultTolerant();
		List<Timestamp> missDate = ft.getLastWeekMissDate();
		ARCHIVE_DBDaoImpl dao = new ARCHIVE_DBDaoImpl();	
		Map<Timestamp, Float> T_t = new HashMap<Timestamp, Float>();
		Map<Timestamp, Float> RH_t = new HashMap<Timestamp, Float>();
	...
	/**
	 * Insert outlier detection output which is CDLOF into OutlierDetectionOutput table
	 * @param hour
	 * @param maxCDLOF
	 */
	public void addOutlierDetectionOutput(int hour, float maxCDLOF){
		OutlierDetectionOutput odop = new OutlierDetectionOutput();
		odop.setOutlierDetectionHour(hour);
		odop.setOutlierDetectionMaxCDLOF(maxCDLOF);
		em.persist(odop);
	}
	
	public void updateOutlierDetectionOutput(int hour, float maxCDLOF){
		Query query = em.createQuery("UPDATE OutlierDetectionOutput t SET t.outlierDetectionMaxCDLOF = :cdlof WHERE t.outlierDetectionHour = :h");
		query.setParameter("cdlof", maxCDLOF);
		query.setParameter("h", hour);
		query.executeUpdate();
	}
	
	public List<OutlierDetectionOutput> getOutlierDetectionOutput(){
		Query query = em.createQuery("SELECT  t FROM OutlierDetectionOutput t ORDER BY t.outlierDetectionHour");
		List<OutlierDetectionOutput> result = query.getResultList();
		return result;
	}
