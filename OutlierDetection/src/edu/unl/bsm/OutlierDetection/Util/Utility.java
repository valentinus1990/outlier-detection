...
//		System.out.println(time);
	 ...
	 * @return
	 */
	public static float getMax(List<WeatherDataForm> list){
		if(list == null){
		...
		for(int i = 0 ; i < list.size(); i++){
			max = (float)list.get(i).getCurrentTemp() > max ? (float)list.get(i).getCurrentTemp() : max;
		}
	 ...
	 * @return
	 */
	public static float getMin(List<WeatherDataForm> list){
		if(list == null){
			throw new NullPointerException("List is null");
		}
		...
		for(int i = 0 ; i < list.size(); i++){
			min = (float)list.get(i).getCurrentTemp() < min ? (float)list.get(i).getCurrentTemp() : min;
		}
