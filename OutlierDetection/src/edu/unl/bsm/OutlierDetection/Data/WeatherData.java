...
public class WeatherData implements IData {
	...
	public void read(List<WeatherDataFinalForm> weatherData){
		for(WeatherDataFinalForm wdata : weatherData){
			
			Date date = (Date)wdata.getDate();
			localTime.put(date, date.toString());
			
			dayofWeek.put(date, wdata.getDayOfWeek());
			hour.put(date, wdata.getHourOfDay());
			T_MAX_t.put(date, wdata.getMaxTemp());
			T_MIN_t.put(date, wdata.getMinTemp());
			T_t.put(date, wdata.getCurTemp());
			if(wdata.getRelativeHum()<=0){
				RH_t.put(date, (float)1);
			}else{
				RH_t.put(date, wdata.getRelativeHum());
			}
			if(wdata.getCoolingLoad() <= 0){
				coolLoad_t.put(date, (float)1);
			}else{
				coolLoad_t.put(date, wdata.getCoolingLoad());
			}
